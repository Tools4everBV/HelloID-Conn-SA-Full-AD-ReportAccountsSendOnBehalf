...
<# Begin: DataSource "AD-user-generate-table-report-mailbox-send-on-behalf" #>
$tmpPsScript = @'
try {
...
Invoke-HelloIDDelegatedForm -DelegatedFormName "AD - Report - Accounts using ""send on behalf"" settings" -DynamicFormGuid $dynamicFormGuid -AccessGroups $delegatedFormAccessGroupGuids -Categories $delegatedFormCategoryGuids -UseFaIcon "True" -FaIcon "fa fa-info-circle" -returnObject ([Ref]$delegatedFormRef) 
<# End: Delegated Form #>
	...
	$tmpScript = @'
            try {
                if($exportReport -eq "True") {
                    ...
                    $exportFile = $HIDreportFolder + "Report_AD_AccountsMailboxSendOnBehalf_" + $timeStamp + ".csv"
...
'@ 
	...
	Invoke-HelloIDAutomationTask -TaskName "AD-export-report-accounts-mailbox-send-on-behalf" -UseTemplate "False" -AutomationContainer "8" -Variables $tmpVariables -PowershellScript $tmpScript -ObjectGuid $delegatedFormRef.guid -ForceCreateTask $true -returnObject ([Ref]$delegatedFormTaskGuid) 
} else {
	Write-ColorOutput Yellow "Delegated form 'AD - Report - Accounts using "send on behalf" settings' already exists. Nothing to do with the Delegated Form task..." 
}
